...
/**
 * Copy of gurux.serial.GXSerial
 *
 * The Serial component determines methods that make the communication
 * possible using serial port connection.
 *
 * Copy was made to extend access visibility permissions.
 */
public class SerialCopy implements IGXMedia, IGXMedia2, AutoCloseable {
     ...
     * Receiver thread.
     */
    private ReceiveThreadCopy receiver;
    /**
     ...
     *            Stop bits.
     */
    public SerialCopy(final String port, final BaudRate baudRateValue, final int dataBitsValue,
    ...
    private static void initialize() {
        if (!initialized) {
            ...
            try (InputStream in = gurux.serial.GXSerial.class.getResourceAsStream(
    ...
    @Override
    public void send(final Object data, final String target) throws Exception {
        if (hWnd == 0) {
    ...
    public final void open() throws Exception {
        ...
        try {
            ...
            setRtsEnable(true);
            setDtrEnable(true);
            receiver = new ReceiveThreadCopy(this, hWnd);
            receiver.start();
